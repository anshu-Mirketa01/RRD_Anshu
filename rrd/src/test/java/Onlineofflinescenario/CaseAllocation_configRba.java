package Onlineofflinescenario;
	...
	import com.relevantcodes.extentreports.LogStatus;
		 ...
		 *
		 */
	public class CaseAllocation_configRba {
			...
			public void setUp() throws FileNotFoundException, InterruptedException {
				report = new ExtentReports("./Report/CaseCreationRba.html");
				logger = report.startTest("Verify Case CreationRba");
			...
			@Test
			public void CaseCreation_configRba() throws InterruptedException
			{
				
				Xls_Reader reader = new Xls_Reader(Testdata_sheet_path);
				
				int rowCount = reader.getRowCount("RBAAllocation");
				for (int rowNum = 2; rowNum <= rowCount; rowNum++)
				{
				     String Status=reader.getCellData("RBAAllocation", "Status", rowNum);
					 String Priority=reader.getCellData("RBAAllocation", "Priority", rowNum);
					 String CaseOrigin=reader.getCellData("RBAAllocation", "CaseOrigin", rowNum);
					 String  saluationtest=reader.getCellData("RBAAllocation", "Salutation", rowNum);
					 String firstname=reader.getCellData("RBAAllocation", "Firstname", rowNum);
					 String lastName=reader.getCellData("RBAAllocation", "LastName", rowNum);
					 String Account=reader.getCellData("RBAAllocation", "Account", rowNum);
					 String Ratingval=reader.getCellData("RBAAllocation", "Ratingtype", rowNum);
					 String RRDType=reader.getCellData("RBAAllocation", "RRDType", rowNum);
					 String RRDCaseReason=reader.getCellData("RBAAllocation", "RRDCaseReason", rowNum);
				     String WebEmailtext=reader.getCellData("RBAAllocation", "WebEmailtext", rowNum);
				     String Subject=reader.getCellData("RBAAllocation", "Subject", rowNum);
				     String Description=reader.getCellData("RBAAllocation", "Description", rowNum);
				     String Internalcommnet=reader.getCellData("RBAAllocation", "Internal comment", rowNum);
				     String Assigneduser=reader.getCellData("RBAAllocation", "ExpectedResult", rowNum);
				     String Teamname=reader.getCellData("RBAAllocation", "TeamName", rowNum);
				     String ToBecount=reader.getCellData("RBAAllocation", "ToBeAssigned", rowNum);
