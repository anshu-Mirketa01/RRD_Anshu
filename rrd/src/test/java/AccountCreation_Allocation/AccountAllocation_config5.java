...
public class AccountAllocation_config5 {
	...
	SoftAssert softassert=new SoftAssert();
		...
		public void setUp() throws FileNotFoundException, InterruptedException {
			report = new ExtentReports("./Report/AccountCreation5.html");
			logger = report.startTest("Verify Account Creation5");
		...
		@Test
		public void AccountCreation_config5() throws InterruptedException
		{
			
			Xls_Reader reader = new Xls_Reader(Testdata_sheet_path);
			
			int rowCount = reader.getRowCount("AccountAllocation5");
			for (int rowNum = 2; rowNum <= rowCount; rowNum++)
			{
			     String AccountName=reader.getCellData("AccountAllocation5", "AccountName", rowNum);
				 String Rating=reader.getCellData("AccountAllocation5", "Rating", rowNum);
				 String Industryval=reader.getCellData("AccountAllocation5", "Industry", rowNum);
				 String Result=reader.getCellData("AccountAllocation5", "ExpectedResult", rowNum);
			     String Teamname=reader.getCellData("AccountAllocation5", "TeamName", rowNum);
			     String ToBecount=reader.getCellData("AccountAllocation5", "ToBeAssigned", rowNum);
			 ...
			 Accobj.scrolltoindustry();
			 logger.log(LogStatus.INFO, "scroll to Industry");
			 
			 Accobj.clickonindustry();
			 logger.log(LogStatus.INFO, "Click on Industry");
